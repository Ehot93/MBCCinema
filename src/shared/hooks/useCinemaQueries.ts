...
export function useCinemaSessions(cinemaId: number, options?: { enabled?: boolean }) {
  return useQuery({
...
export function useCinemaDetails(cinemaId: number, options?: { enabled?: boolean }) {
  // Предзагружаем данные параллельно
  const cinemasQuery = useCinemas();
  const moviesQuery = useMovies();
  const sessionsQuery = useCinemaSessions(cinemaId, { enabled: options?.enabled });
...
export function usePrefetchCinemaData() {
  ...
  const prefetchCinemaDetails = useCallback(
    (cinemaId: number) => {
      queryClient.prefetchQuery({
        queryKey: queryKeys.cinemaSessions(cinemaId),
        queryFn: () => api.fetchCinemaSessions(cinemaId),
